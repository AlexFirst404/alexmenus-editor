...
- **Анимации открытия:** темп `speed: 1..100` + порядок `sweep` / `rows` / `random` / `corners` (из четырёх
  углов) / `edges` (сверху и снизу к центру) / `ellipse` (эллипс из центра к краям).
- **`/am reload`** перезагружает плагин целиком.
- **«Применить» прямо из редактора:** `/am editor` открывает live-сессию — кнопка в браузере отправляет
  правки на сервер, он сам пишет файлы и перезагружается. «Сохранить и получить код» (`/am apply <код>`)
...
- **Редактор:** **пикер всех блоков/предметов 1.21.11** (поиск+иконки), **точные иконки как в майне**
  ...
  **node-граф условий**, панель requirements (view/click/open), граф навигации (зум колёсиком),
  **светлая тема**, **сырой YAML с подсветкой и нумерацией строк**, свои модалки/чекбоксы,
